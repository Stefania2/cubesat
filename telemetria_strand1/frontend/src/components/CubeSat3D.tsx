...
export function CubeSat3D({ estado, lecturas, girando }: Props) {
  return (
    <Canvas camera={{ position: [5, 2.5, 5], fov: 45 }} dpr={[1, 2]}>
      ...
      {/*
        Nada de esta escena carga recursos: son mallas, luces y una geometria de
        puntos generada en el cliente. Por eso no hace falta ninguna frontera de
        suspension aqui. Si mas adelante se anade algo que si cargue --- una
        fuente para texto 3D, una textura, un modelo glTF ---, hay que envolverlo
        en su propio `Suspense`: sin eso la suspension sube hasta la frontera de
        la ruta y React oculta la pagina entera con `display:none !important`
        mientras espera, dejando un esqueleto indefinido con el lienzo ya
        montado detras.
      */}
